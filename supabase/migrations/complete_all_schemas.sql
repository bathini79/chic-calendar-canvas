...
CREATE OR REPLACE FUNCTION "auth"."create_profile"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    -- One simple statement to create a profile with user_id = id
...
ALTER FUNCTION "auth"."create_profile"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "auth"."create_profile_on_signup"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $_$
DECLARE
    default_role text := 'customer';
BEGIN
    -- Simple insert - ID only first
    INSERT INTO public.profiles (id, user_id)
    VALUES (NEW.id, NEW.id)
    ON CONFLICT (id) DO NOTHING;
    
    -- Update other fields in a separate statement
    ...
    BEGIN
        IF NEW.raw_user_meta_data ? 'role' THEN
            EXECUTE 'UPDATE public.profiles SET role = $1::public.user_role WHERE id = $2'
            USING COALESCE(NEW.raw_user_meta_data->>'role', default_role), NEW.id;
        ELSE
            EXECUTE 'UPDATE public.profiles SET role = $1::public.user_role WHERE id = $2'
            USING default_role, NEW.id;
        END IF;
    EXCEPTION WHEN OTHERS THEN
        -- If role casting fails, use default
        EXECUTE 'UPDATE public.profiles SET role = $1::public.user_role WHERE id = $2'
        USING default_role, NEW.id;
        RAISE NOTICE 'Using default role: %', default_role;
    END;
...
ALTER FUNCTION "auth"."create_profile_on_signup"() OWNER TO "postgres";
...
REVOKE ALL ON FUNCTION "auth"."create_profile"() FROM PUBLIC;
GRANT ALL ON FUNCTION "auth"."create_profile"() TO "service_role";
...
GRANT SELECT,INSERT ON TABLE "auth"."users" TO "authenticator";
