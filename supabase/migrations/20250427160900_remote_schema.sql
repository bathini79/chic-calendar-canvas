...
alter table "auth"."users" add column "password" text;
...
create policy "Allow user selection"
on "auth"."users"
as permissive
for select
to authenticated
...
CREATE TRIGGER after_user_create AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION create_profile();

CREATE TRIGGER after_user_delete AFTER DELETE ON auth.users FOR EACH ROW EXECUTE FUNCTION delete_profile();
