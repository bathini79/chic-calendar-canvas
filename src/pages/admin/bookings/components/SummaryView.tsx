...
export const SummaryView: React.FC<SummaryViewProps> = ({ appointmentId }) => {
  ...
  const renderTransactionCard = (transaction: Appointment, isRefund: boolean = false) => {
    return (
        ...
        <CardContent className="p-6 space-y-6">
          <div className="flex items-center justify-between border-b pb-4">
            <div className="flex-1">
              <div className={`inline-flex items-center px-2.5 py-1 rounded ${
                ...
                {isRefund ? (
                  ...
                  <><CheckCircle2 className="h-4 w-4 mr-1" />Sale #{transaction.id.slice(0, 8)}</>
                )}
              </div>
            ...
            <div className="text-xl font-semibold">
              {isRefund ? (
                <span className="text-red-600">-₹{Math.abs(transaction.total_price).toFixed(2)}</span>
              ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 pt-4 border-t">
              ...
              <div className="flex justify-between text-lg font-bold">
                <span>Total</span>
                <span>{isRefund ? '-' : ''}₹{Math.abs(transaction.total_price).toFixed(2)}</span>
              </div>
            </div>

            {/* Additional Information for Refunds */}
            {isRefund && (
  ...
  // Sort refunds by created_at in descending order (latest first)
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          {sortedRefunds.map(refund => renderTransactionCard(refund, true))}
          {/* Original sale is always shown last */}
          {renderTransactionCard(originalSale)}
        </div>
      </div>
