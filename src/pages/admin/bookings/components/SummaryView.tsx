        ...
        <div className="overflow-y-auto">
          ...
          {items &&
            items.map((item, idx) => (
              <div
                key={idx}
                className="py-2 flex justify-between items-start border-b"
              >
                <div className="flex-1">
...
export const SummaryView: React.FC<SummaryViewProps> = ({
  ...
  const [refundReason, setRefundReason] = React.useState<RefundData["reason"]>(
  ...
  const handleSaveEdits = async () => {
    ...
    try {
      const updatePromises = Object.entries(selectedEmployees).map(
  ...
  if (transactionDetails) {
    const getGroupedBookings = (transaction: any) => {
      ...
      const serviceBookings = transaction.bookings.filter(
        (b) => b.service_id && !b.package_id
      );
      ...
      return result;
    };    // DEPRECATED: Function to handle restoring referral wallet credits
    // This function is no longer used as referral wallet refunds are now handled in useAppointmentActions.ts
    // to prevent double-crediting the customer's referral wallet
    const handleReferralWalletRefund = async (
      originalTransaction,
      refundProportion
    ) => {
      try {
        // Check if there was referral wallet used in the original transaction
        const referralWalletAmount =
          originalTransaction.referral_wallet_redeemed || 0;

        if (referralWalletAmount <= 0) {
          return; // No referral wallet was used in this transaction
        }

        // Calculate amount to refund based on the proportion of the total being refunded
        const referralWalletToRefund = referralWalletAmount * refundProportion;

        if (referralWalletToRefund <= 0) {
          return; // No need to refund
        }

        // Get the customer's current referral wallet balance
        const { data: customerData, error: fetchError } = await supabase
          .from("profiles")
          .select("referral_wallet")
          .eq("id", originalTransaction.customer_id)
          .single();

        if (fetchError) {
          console.error("Error fetching customer referral wallet:", fetchError);
          return;
        }

        // Calculate new balance
        const currentBalance = customerData.referral_wallet || 0;
        const newBalance = currentBalance + referralWalletToRefund;

        // Update the customer's referral wallet
        ...
        if (updateError) {
          console.error("Error updating referral wallet:", updateError);
          return;
        }

        console.log(
          `Referral wallet restored: ${currentBalance} -> ${newBalance} (Added: ${referralWalletToRefund})`
        );
        return referralWalletToRefund;
      } catch (error) {
    ...
    const handleRefundSale = async () => {
      ...
      try {
        ...
        // Calculate the refund proportion for referral wallet refund
        const allBookingsAmount =
          transactionDetails.originalSale.bookings.reduce(
            (total, booking) => total + (booking.price_paid || 0),
            0
          );        // Calculate raw refund amount based on booking prices
        const refundAmount = transactionDetails.originalSale.bookings
          .filter((booking) => selectedBookingIds.includes(booking.id))
          .reduce((total, booking) => total + (booking.price_paid || 0), 0);

        // Calculate the proportion of the total being refunded
        const refundProportion =
          allBookingsAmount > 0 ? refundAmount / allBookingsAmount : 0;
            // Calculate the actual discounted amount being refunded
        const originalTotalPrice = Math.abs(transactionDetails.originalSale.total_price || 0);
        const discountedRefundAmount = originalTotalPrice * refundProportion;

        // NOTE: We're NOT calling handleReferralWalletRefund here anymore
        // as it's already being handled in processRefund function
        // This prevents referral wallet credits from being doubled
        ...
        if (success) {
          await loadAppointmentDetails();          setShowRefundDialog(false);          // Check if the original transaction used referral wallet
          const referralWalletAmount =
            transactionDetails.originalSale.referral_wallet_redeemed || 0;
          
          // Calculate the actual discounted amount being refunded
          const actualRefundAmount = originalTotalPrice * refundProportion;
          
          if (referralWalletAmount > 0) {
            const refundedAmount =
              (refundAmount / allBookingsAmount) * referralWalletAmount;
            toast.success(
              `Refund of ₹${actualRefundAmount.toFixed(2)} processed successfully. Referral wallet credits have been restored to the customer.`,
              { duration: 5000 }
            );
          } else {
            toast.success(`Refund of ₹${actualRefundAmount.toFixed(2)} processed successfully`);
          }
        }
      } catch (error: any) {
    ...
    const handleVoidSale = async () => {
      ...
      try {
        const bookingIds = transactionDetails.originalSale.bookings.map(
          (booking) => booking.id
        );
    ...
    const allTransactions = [...refunds, originalSale].sort(
      (a, b) =>
      ...
      <>
        <div className="space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto px-1">
          {allTransactions.map((transaction) => {
            ...
            const subtotal = transaction.bookings.reduce(
              (sum, booking) => sum + (booking.price_paid || 0),
              0
            );
                      ...
                      <div className="flex items-center gap-2">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-56">
                            ...
                            <DropdownMenuItem
                              onSelect={() => setShowEditSaleDialog(true)}
                            >
                            ...
                            <div className="pl-6 border-l-2 border-gray-300 ml-4 mt-2 space-y-1">
                              {item.bookings.map((booking: any) => {
                                ...
                                return (
                                  <div
                                    key={booking.id}
                                    className="py-1 flex justify-between items-start"
                                  >
                                    <div className="flex-1">
                  ...
                  <div className="space-y-1 pt-2 border-t">
                    ...
                    {transaction?.membership_discount > 0 ? (
                      <div className="flex justify-between text-sm text-green-600">
                        ...
                        <span>
                          -₹{transaction.membership_discount.toFixed(2)}
                        </span>
                      </div>
                    ) : null}
                    <div className="flex justify-between text-sm text-green-600">
                      ...
                      <span>
                        {transaction?.discount_value > 0
                          ? transaction.discount_type === "percentage"
                            ? `-₹${(
                                subtotal *
                                (transaction.discount_value / 100)
                              ).toFixed(2)}`
                            : `-₹${transaction.discount_value.toFixed(2)}`
                          : null}
                      </span>
                    </div>{" "}
                    {/* Regular referral wallet usage in sale transaction */}
                    {transaction?.referral_wallet_redeemed > 0 && !isRefund && (
                      <div className="flex justify-between text-sm text-amber-600">
                        <span className="flex items-center gap-1">
                          <Star className="h-3 w-3" />
                          Referral Wallet Used
                        </span>
                        <span>
                          -₹{transaction.referral_wallet_redeemed.toFixed(2)}
                        </span>
                      </div>
                    )}
                    {/* Refunded referral wallet credits indicated in refund transaction */}
                    {transaction?.referral_wallet_redeemed < 0 && isRefund && (
                      <div className="flex justify-between text-sm text-green-600">
                        <span className="flex items-center gap-1">
                          <Star className="h-3 w-3" />
                          Referral Wallet Credited
                        </span>
                        <span>
                          +₹
                          {Math.abs(transaction.referral_wallet_redeemed).toFixed(
                            2
                          )}
                        </span>
                      </div>
                    )}
                    {transaction?.round_off_difference ? (
                      <div className="flex justify-between text-sm">
                        <span>Round off</span>
                        <span>
                          ₹{formatPrice(transaction?.round_off_difference)}
                        </span>
                      </div>
                    ) : (
                      ""
                    ...
                    <div className="space-y-2 pt-4 border-t">
                      {transaction.refund_reason && (
                        <div>
                          ...
                          <p className="text-gray-600">
                            {formatRefundReason(transaction.refund_reason)}
                          </p>
                        </div>
                      )}
                      ...
                      {/* Display referral wallet refund information */}
                      {transaction.referral_wallet_redeemed < 0 && (
                        <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded-md">
                          <p className="font-medium text-xs flex items-center gap-1 text-amber-800">
                            <Star className="h-3 w-3" />
                            Referral Wallet Credits Restored
                          </p>
                          <p className="text-amber-700 text-sm font-medium">
                            +₹
                            {Math.abs(
                              transaction.referral_wallet_redeemed
                            ).toFixed(2)}
                          </p>
                        </div>
                      )}
        ...
        <Dialog open={showVoidDialog} onOpenChange={setShowVoidDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Are you sure you want to void this sale? This action cannot be
                undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="py-4 space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const allBookingIds =
                        transactionDetails.originalSale.bookings
              ...
              <div className="max-h-36 overflow-y-auto space-y-1">
                {transactionDetails.originalSale.bookings
                  .filter((booking) => booking.status !== "refunded")
                  .map((booking) => {                    const itemName =
                      booking.service?.name || booking.package?.name;
                    const itemPrice = booking.price_paid;
                    
                    // Calculate the proportion this item represents of the total booking amount
                    const totalBookingAmount = transactionDetails.originalSale.bookings.reduce(
                      (sum, b) => sum + (b.price_paid || 0), 0
                    );
                    const itemProportion = itemPrice / totalBookingAmount;
                    const discountedPrice = (transactionDetails.originalSale.total_price || 0) * itemProportion;
                    const discountedItemPrice = Math.abs(discountedPrice);
                      ...
                      >
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            <span className={itemPrice !== discountedItemPrice ? "line-through mr-2 text-gray-400" : ""}>
                              ₹{itemPrice.toFixed(2)}
                            </span>
                            {itemPrice !== discountedItemPrice && 
                              <span className="text-green-600">₹{discountedItemPrice.toFixed(2)}</span>
                            }
                          </p>
                          {booking.employee && (
                            <p className="text-sm text-gray-500">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select value={refundedBy} onValueChange={setRefundedBy}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={refundReason}
                    onValueChange={(value) =>
                      setRefundReason(value as RefundData["reason"])
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="health_concern">
                        Health Concern
                      </SelectItem>
                      <SelectItem value="price_dispute">
                        Price Dispute
                      </SelectItem>
        ...
        <Dialog open={showEditSaleDialog} onOpenChange={setShowEditSaleDialog}>
          <DialogContent className="max-w-3xl">
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setShowEditSaleDialog(false)}
              >
                Cancel
              </Button>
