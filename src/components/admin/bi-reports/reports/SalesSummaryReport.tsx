...
import { BarChart3, Package, Filter, Calendar as CalendarIcon, MapPin, X, Users, ShoppingCart, Layers, Clock } from 'lucide-react';
...
export function SalesSummaryReport() {
  ...
  // Set up export data
  useEffect(() => {
    setExportData(summarizedData);
    setReportName('sales-summary');
  }, [summarizedData, setExportData, setReportName]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4 sm:space-y-0">
        {/* Type (Group By), Calendar, and Filter aligned to the left */}
        <div className="flex items-center justify-start gap-3">
          {/* Type dropdown first */}          <Select value={groupBy} onValueChange={(value: GroupByOption) => setGroupBy(value)}>
          ...
          {/* Filter button third - icon only on mobile, with text on desktop */}
          <Button 
            variant="outline" 
            size="sm" 
      ...
      <ReportDataTable
        ...
        loading={isLoading}
        externalSearchTerm=""
        totalCount={summarizedData.length}
      />
    </div>
  );
}
