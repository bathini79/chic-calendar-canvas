...
export function DateRangePicker({
  ...
  }, [dateRange]);  // Common form inputs for both versions
  const DateInputs = () => (
    <div className="grid grid-cols-2 gap-4">
      <div>
        ...
        <Input
          ...
          className="w-full focus:border-primary/50"
  ...
  const PresetSelector = () => (
    <div>
      ...
      <Select value={selectedPreset} onValueChange={handlePresetChange}>
        <SelectTrigger className="w-full border-muted/30 focus:ring-primary/20">
          <SelectValue placeholder="Select a preset range" />
        </SelectTrigger>
        <SelectContent className="border-muted/30">
  ...
  const CalendarSection = () => {
    ...
    const calendarClassNames = {
      day: "h-10 w-10 p-0 font-normal hover:bg-primary/10 relative flex items-center justify-center",
      day_selected: "bg-primary/80 text-primary-foreground hover:bg-primary hover:text-primary-foreground rounded-full",
      day_today: "border border-primary/50 text-accent-foreground font-medium",
      ...
      month: "space-y-2",
      nav_button: "h-8 w-8 hover:bg-muted rounded-full flex items-center justify-center bg-background hover:bg-accent/20",
      head_row: "grid grid-cols-7 mb-2",
      row: "grid grid-cols-7 mt-0",
      cell: "text-center p-0 relative [&:has([aria-selected])]:bg-primary/5 first:[&:has([aria-selected])]:rounded-l-full last:[&:has([aria-selected])]:rounded-r-full focus-within:relative focus-within:z-20",
    ...
    return (
      <div className={`mt-4 ${isMobile ? 'space-y-6' : 'grid grid-cols-2 gap-8 justify-center'}`}>
        {/* First calendar */}
        <div className={isMobile ? '' : 'flex justify-end'}>
          <Calendar
            ...
            initialFocus
            defaultMonth={tempDateRange?.from || new Date()}            className="w-full border-0 bg-background rounded-lg p-2"
  ...
  const mobileContent = (
    <div className="space-y-6 pb-20">
      <PresetSelector />
      <DateInputs />
      <CalendarSection />      <div className="fixed bottom-0 left-0 right-0 bg-white py-4 px-4 pb-8 border-t shadow-lg">
        <Button 
          onClick={handleApply} 
          className="w-full h-12 text-base font-medium bg-primary hover:bg-primary/90 rounded-lg transition-colors"
        >
          Apply
        </Button>
      </div>
  ...
  const desktopContent = (
    <div className="space-y-5">
      ...
      <DateInputs />
      <CalendarSection />      <div className="flex justify-end mt-6">
        <div className="flex space-x-3">
          ...
          <Button 
            onClick={handleApply} 
            className="font-medium bg-primary hover:bg-primary/90 rounded-lg px-5"
          >
            Apply
          </Button>
        </div>
    ...
    <div className={cn("flex items-center", className)}>
      {isMobile ? (
        ...
        <Popover open={isPopoverOpen} onOpenChange={(open) => {
          ...
          <PopoverContent className="w-[720px] p-6 shadow-lg border border-muted/30 rounded-lg" align={align} side="bottom" sideOffset={5}>
            {desktopContent}
          </PopoverContent>
        </Popover>
      )}
